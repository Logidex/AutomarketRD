...
export default function PlanCard({
  ...
  const acento = ESTILO_ACENTO[plan.nivel] ?? "text-gray-400";
          ...
          {[1, 2, 3, 4].map((n) => (
            <div
              ...
              className={`h-1.5 flex-1 rounded-full ${
                n <= prioridad.nivel ? acento.replace("text-", "bg-") : "bg-white/10"
              }`}
            />
          ))}
        </div>
...
function FilaBeneficio({ texto, icono }: { texto: string; icono: React.ReactNode }) {
  return (
    <div className="flex items-start gap-2">
      ...
      <span className="text-sm text-gray-300">{texto}</span>
    </div>
  );
}
